            ...
            if observation.shape == observation_space.shape:
                ...
                raise ValueError(
                    "Error: Unexpected observation shape {} for ".format(
                        observation.shape) + "Box environment, please use {} ".
...
class SimpleRLModel(BaseRLModel):
    ...
    def evaluate(self, n_episodes):
        ...
        success_reward = 0
        if hasattr(self, 'reward_mode') and self.reward_mode == 1:
            success_reward = 1
        ...
        for _ in range(n_episodes):
            ...
            while not done and reward < 1.:
                ...
                total_reward += rew
                steps += 1
            results.append((float(np.allclose(rew, success_reward)), steps,
                            total_reward))
        return results
                ...
                for _ in range(total_timesteps // self.n_envs):
                    ...
                    for idx in np.argwhere(dones):
                        ...
                        if self.eval_env is not None and num_episodes % self.eval_every == 0:
                            ...
                            summary = tf.Summary(value=[
                                tf.Summary.Value(
                                    tag="test_reward/steps",
                                    simple_value=test_steps[-1])
                            ])
                            writer.add_summary(summary, num_episodes)
                            summary = tf.Summary(value=[
                                tf.Summary.Value(
                                    tag="test_reward/rewards",
                                    simple_value=test_rewards[-1])
                            ])
                            writer.add_summary(summary, num_episodes)
                            if (len(test_successes) + 1) % 20 == 0:
                                print(
                                    "Evaluation perf for last 100 evaluations: {}"
                                    .format(mean_eval))
                            ...
                            # Also store the replay buffer statistics if using HER
                            if hasattr(
                                    self, 'replay_buffer_hindsight'
                            ) and self.replay_buffer_hindsight is not None and hasattr(
                                    self.eval_env, 'is_goal_in_desired'):
                                assert (hasattr(self, 'replay_buffer')
                                        and self.replay_buffer is not None)
                                assert (hasattr(self, 'hindsight_frac'))
                                # Sample K transitions from the replay buffer and compute goal statistics
                                num_transitions = 2500
                                assert (self.replay_buffer_hindsight.size >=
                                        num_transitions)
                                batch_desired_goals = self.replay_buffer_hindsight.sample(
                                    num_transitions
                                )[-1]  # Assuming that desired goal is the last element in transition list
                                goals_in_desired = self.eval_env.is_goal_in_desired(
                                    batch_desired_goals)
                                # Weighted sum of the number of goals in desired dist from hindsight buffer and real replay buffer
                                desired_goal_percentage = self.hindsight_frac * (
                                    np.sum(goals_in_desired) / num_transitions
                                ) + (
                                    1 - self.hindsight_frac
                                )  # Assuming that all goals in self.replay_buffer came from desired goal
                                summary = tf.Summary(value=[
                                    tf.Summary.Value(
                                        tag=
                                        "replay_buffer/desired_goal_percentage",
                                        simple_value=desired_goal_percentage)
                                ])
                                writer.add_summary(summary, self.global_step)
                                summary = tf.Summary(value=[
                                    tf.Summary.Value(
                                        tag="replay_buffer/hindsight_frac",
                                        simple_value=self.hindsight_frac)
                                ])
                                writer.add_summary(summary, self.global_step)
                        ...
                        if self.verbose >= 1 and log_interval is not None and num_episodes % log_interval == 0:
                            if len(legacy_episode_rewards[-101:-1]) == 0:
                                ...
                                mean_100ep_reward = round(
                                    float(
                                        np.mean(
                                            legacy_episode_rewards[-101:-1])),
                                    2)

                            logger.record_tabular("steps", self.task_step)
                            logger.record_tabular("episodes", num_episodes)
                            logger.record_tabular("repl_buff_len",
                                                  len(self.replay_buffer))
                            if hasattr(
                                    self, 'replay_buffer_hindsight'
                            ) and self.replay_buffer_hindsight is not None:
                                logger.record_tabular(
                                    "repl_buff_hindsight_len",
                                    len(self.replay_buffer_hindsight))
                            if hasattr(
                                    self, 'landmark_generator'
                            ) and self.landmark_generator is not None:
                                logger.record_tabular(
                                    "landmark_gen_length",
                            ...
                            if self.exploration:
                                logger.record_tabular(
                                    ...
                                    int(100 * self.exploration.value(
                                        self.task_step)))
                            logger.dump_tabular()
