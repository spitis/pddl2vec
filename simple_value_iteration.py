...
from stable_baselines.common.replay_buffer import (
    ReplayBuffer, EpisodicBuffer, her_final, her_future, her_future_landmark,
...
class SimpleValueIteration(SimpleRLModel):
    ...
    def forward_pass(self, obs):
        ...
        num_state_facts = tf.count_nonzero(
            tf.less(all_state_facts, len(self.env.task.facts)), 1)
        num_state_facts = tf.cast(tf.expand_dims(num_state_facts, 1), tf.int32)
        ...
        state_vals = state_embeds * self.W_value_network + self.b_value_network
        return state_vals
    ...
    def _setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                # Placeholders.
                ob_space = self.env.observation_space.spaces["observation"]
                self.obs_ph = tf.placeholder(
                    shape=[None, ob_space.n], dtype=tf.int32, name='obs_ph')
                self.obs_next_ph = tf.placeholder(
                    shape=[None, None, ob_space.n],
                    dtype=tf.int32,
                    name='obs_next_ph')
                self.batch_size = tf.shape(self.obs_ph)[0]
                ...
                with tf.variable_scope("embedding_network"):
                    ...
                    self.W_fact_embed = tf.concat([W_fact_embed, zeros], 1)
                ...
                with tf.variable_scope("deep_value_iteration"):
                    ...
                    successor = epsilon_greedy_wrapper(epsilon_ph)
                ...
                with tf.variable_scope("loss"):
                    ...
                    gradients = optimizer.compute_gradients(
                        mean_l2_loss, var_list=policy.trainable_vars)
